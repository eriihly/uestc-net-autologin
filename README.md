# 校园网自动登录（锐捷 SAM + CAS-SSO）

一键完成校园网网页认证：**纯 HTTP 请求，约 1~2 秒完成**，无需打开浏览器。
适用于采用「锐捷 SAM + CAS-SSO 单点登录 + ePortal」这套认证体系的校园网。
...
- ❌ 不需要浏览器、浏览器内核、Node.js、管理员权限、Git
- ✅ 控制台可先启动（依赖为懒加载），装 pip 包之前就能打开页面、用「一键配置环境」安装
...
- 登录表单在 **iframe** 内（本项目直接请求 iframe 的地址，绕过整个页面加载）
- POST 表单必须包含 `type=UsernamePassword` 字段，缺失会返回 500
- 部分环节会跳转运营商站点（如 `www.189.cn`，带反爬验证），认证流程本身**不依赖**它
